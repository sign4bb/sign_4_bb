...
def res_json_2_tbl_latex(json_file):
    ...
    for dset, dset_res in res.items():
        ...
        att_ns = [_.replace('Attack','').replace('Sign', 'SignHunter').replace('Bandit', 'Bandits$_{TD}$').replace('ZOSignHunter', 'ZOSign') for _ in att_ns]
...
def config_json_2_tbl_latex(json_files):
    ...
    for json_file in json_files:
        ...
        _df = pd.DataFrame({r"""\bf{Hyperparameter}""": hparams, r"""\texttt{{{}}} $\ell_{{{}}}$""".format(dset_name, p): vals}).set_index(r"""\bf{Hyperparameter}""")
